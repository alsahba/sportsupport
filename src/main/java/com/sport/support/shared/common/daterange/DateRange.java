...
public record DateRange(LocalDateTime from, LocalDateTime to) {
   ...
   public boolean isValid() {
      if (from.isAfter(to)) {
         throw new BusinessRuleException(DateRangeErrorMessages.ERROR_RANGE_START_DATE_AFTER_END_DATE);
      }
      if (from.isAfter(LocalDateTime.now())) {
         throw new BusinessRuleException(DateRangeErrorMessages.ERROR_RANGE_START_DATE_CANNOT_BE_LATER_THAN_NOW);
      }
